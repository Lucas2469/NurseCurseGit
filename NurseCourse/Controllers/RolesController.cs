...
namespace NurseCourse.Controllers
{
    ...
    public class RolesController : Controller
    {
        ...
        private readonly string _auth0ApiUrl = "https://dev-r3d1ami1jr1ck5ez.us.auth0.com/api/v2/roles";
        ...
        private async Task<ICollection<string>> ObtenerAssignedRoleIdsAsync(string userId)
        {
            var client = _httpClientFactory.CreateClient();
            var token = await _auth0Service.GetAuth0TokenAsync();
            var request = new HttpRequestMessage(HttpMethod.Get, $"https://dev-r3d1ami1jr1ck5ez.us.auth0.com/api/v2/users/{userId}/roles");
            request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
        ...
        private async Task<VUser> ObtenerUsuarioPorIdAsync(string userId)
        {
            var client = _httpClientFactory.CreateClient();
            var token = await _auth0Service.GetAuth0TokenAsync();
            var request = new HttpRequestMessage(HttpMethod.Get, $"https://dev-r3d1ami1jr1ck5ez.us.auth0.com/api/v2/users/{userId}");
            request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
        ...
        public async Task<IActionResult> AssignRole(string userId, string[] selectedRoleIds, string action)
        {
            try
            {
                ...
                var client = _httpClientFactory.CreateClient();
                var token = await _auth0Service.GetAuth0TokenAsync();
                var baseUrl = "https://dev-r3d1ami1jr1ck5ez.us.auth0.com/api/v2/users";
